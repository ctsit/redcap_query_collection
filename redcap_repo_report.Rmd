...
redcap_repo_data <- scrape_redcap_repo_data(number_of_entries = 128, create_csv = F) 
```
...
last_module_update <- redcap_repo_data %>%
  ...
  select(title, version, date_added)

kable(last_module_update, booktabs = T, 
      caption = "Last updated date for modules published by CTS-IT") %>% 
